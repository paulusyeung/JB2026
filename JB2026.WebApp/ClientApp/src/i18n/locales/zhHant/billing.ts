export const billingMessages = {
  statement: {
    title: '帳單結單',
    subtitle: '來自 Invoice Ninja 的客戶結餘清單，供後續結單動作使用',
    lookup: '客戶搜尋',
    dialog: {
      title: '客戶結單',
      subtitle: '為 {client} 產生結單',
      fields: {
        dateRange: '日期範圍',
        status: '狀態',
        selectors: '選項',
      },
      selectors: {
        credits: '貸項',
        payments: '付款',
        aging: '帳齡',
      },
      presets: {
        allOutstanding: '全部未結',
        thisMonth: '本月',
        lastMonth: '上月',
        thisQuarter: '本季',
        thisYear: '本年度',
        all: '全部',
        paid: '已付款',
        unpaid: '未付款',
      },
      actions: {
        close: '關閉',
        cancel: '取消',
        proceed: '繼續',
      },
    },
    actions: {
      columns: '欄位',
      sorting: '排序',
      sortBy: '排序欄位',
      asc: '升冪',
      desc: '降冪',
      checkbox: '核取方塊',
      views: '檢視',
      detailView: '明細檢視',
      cardView: '卡片檢視',
      statement: '結單',
      selected: '已選 {count} 筆',
    },
    headers: {
      clientName: '客戶',
      clientCode: '客戶編號',
      externalClientId: '客戶 ID',
      outstandingBalance: '未結餘額',
    },
    labels: {
      empty: '-',
    },
    messages: {
      loadFailed: '無法載入帳單結單客戶清單。',
      selectSingleClient: '請選取且只選取一個客戶核取方塊以啟用結單。',
      previewBlocked: '無法開啟結單預覽。請允許此網站的彈出視窗後再試。',
      previewLoading: '正在載入結單預覽...',
      previewTitle: '正在開啟結單...',
      launchFailed: '啟動客戶結單失敗。',
      launchUnexpected: '啟動客戶結單時發生未預期錯誤。',
      pendingAction: '結單動作將於後續變更中加入。',
    },
  },
  invoiceStats: {
    title: '發票統計',
    subtitle: '未選日期範圍時，預設顯示本年度資料。',
    startDate: '開始日期',
    endDate: '結束日期',
    rows: '筆數：{count}',
    exportToExcel: '匯出到 Excel',
    blank: '(空白)',
    unknownPeriod: '未知',
    empty: '找不到本年度已發送的發票。可選擇日期範圍以擴大結果。',
    emptyFiltered: '找不到所選日期範圍內已發送的發票。',
    mobilePreferredNotice: '樞紐分析較適合桌面版。行動版會顯示精簡摘要與發票清單。',
    summary: {
      invoices: '發票數',
      invoiceAmount: '發票金額',
    },
    loadFailed: '無法載入帳單發票統計。',
  },
  clients: {
    title: '帳單客戶',
    subtitle: '已與 Invoice Ninja 同步的客戶記錄',
    lookup: '客戶搜尋',
    actions: {
      columns: '欄位',
      sorting: '排序',
      sortBy: '排序欄位',
      asc: '升冪',
      desc: '降冪',
      checkbox: '核取方塊',
      views: '檢視',
      detailView: '明細檢視',
      cardView: '卡片檢視',
      newClient: '新增客戶',
      selected: '已選 {count} 筆',
    },
    headers: {
      clientName: '客戶',
      clientCode: '客戶編號',
      externalClientId: '客戶 ID',
      outstandingBalance: '未結餘額',
    },
    labels: {
      empty: '-',
    },
    messages: {
      loadFailed: '無法載入帳單客戶清單。',
      loadRecordFailed: '無法載入所選客戶記錄。',
      migrateSuccess: '客戶已成功同步到 Invoice Ninja。',
      migrateFailed: '同步客戶到 Invoice Ninja 失敗：{error}',
      alreadySynced: '此客戶已同步到 Invoice Ninja。',
    },
    form: {
      newTitle: '新增客戶',
      editTitle: '客戶記錄',
      migrateCustomer: '遷移客戶',
      migrateCustomerSelect: '選擇要遷移的 JB2026 客戶',
      alreadySyncedNotice: '此客戶已同步到 Invoice Ninja，可更新其帳單資訊。',
      customerName: '客戶名稱',
      customerCode: '客戶代號',
      group: '群組',
      billTo: 'Bill To',
      shipToAddress: 'Ship To 地址',
      readiness: '遷移就緒狀態',
      requiredName: '客戶名稱',
      requiredCode: '客戶代號',
      requiredBillTo: 'Bill To 地址',
      editCustomer: '編輯客戶',
      editHint: '此客戶缺少所需的帳單資料。請編輯客戶記錄以補齊。',
      migrate: '遷移',
      update: '更新',
      cancel: '取消',
      loadCustomersFailed: '無法載入客戶清單。',
    },
  },
  invoices: {
    title: '帳單發票',
    subtitle: '已與 Invoice Ninja 同步的發票摘要',
    lookup: '客戶查詢',
    invoiceLookup: '發票查詢',
    actions: {
      columns: '欄位',
      sorting: '排序',
      sortBy: '排序欄位',
      asc: '升冪',
      desc: '降冪',
      checkbox: '核取方塊',
      views: '檢視',
      detailView: '明細檢視',
      cardView: '卡片檢視',
      newInvoice: '新增發票',
      markSent: '標記為已發送',
      download: '下載',
      uploadToDms: '上傳至 DMS',
      invoicePdf: '發票',
      deliveryNote: '送貨單',
      confirmMarkSent: '確認標記為已發送',
      cancel: '取消',
      markAsSent: '標記為已發送',
    },
    headers: {
      invoice: '發票',
      client: '客戶',
      invoiceDate: '發票日期',
      status: '狀態',
      amount: '金額',
      dueDate: '到期日',
      lastSynced: '最後同步',
    },
    labels: {
      amount: '金額',
      due: '到期',
      lastSynced: '最後同步',
      selected: '已選 {count} 筆',
      empty: '-',
    },
    status: {
      unknown: '未知',
      draft: '草稿',
      sent: '已發送',
      viewed: '已檢視',
      partial: '部分付款',
      paid: '已付款',
      cancelled: '已取消',
      reversed: '已沖銷',
      overdue: '已逾期',
      unpaid: '未付款',
      deleted: '已刪除',
    },
    messages: {
      loadFailed: '無法載入帳單發票。',
      sendFailed: '發送發票失敗。',
      sendUnexpected: '發送發票時發生未預期錯誤。',
      previewBlocked: '無法開啟文件預覽。請允許此網站的彈出視窗後再試。',
      previewLoading: '正在載入文件預覽...',
      previewTitle: '正在開啟 PDF...',
      downloadInvoicePdfFailed: '下載發票 PDF 失敗。',
      downloadInvoicePdfUnexpected: '下載發票 PDF 時發生未預期錯誤。',
      downloadDeliveryNoteFailed: '下載送貨單失敗。',
      downloadDeliveryNoteUnexpected: '下載送貨單時發生未預期錯誤。',
      dmsUploadSuccess: '文件已上傳至 DMS「{title}」。',
      dmsAlreadyExists: 'DMS 中已有標題為「{title}」的文件。',
      dmsUploadFailed: '目前無法將文件上傳至 DMS。',
      markSentConfirm: '確定要將此發票標記為已發送到 Invoice Ninja 嗎？此操作無法復原。',
    },
    editor: {
      titleCreate: '新建發票',
      titleEdit: '編輯發票',
      titleView: '查看發票',
      badge: {
        create: '新建',
        edit: '草稿',
        view: '唯讀',
      },
      fields: {
        client: '客戶',
        clientPlaceholder: '搜尋客戶…',
        invoiceDate: '發票日期',
        dueDate: '到期日期',
        jobNumber: '工單編號',
      },
      lineItems: {
        poNumber: '採購訂單號',
        description: '描述',
        qty: '數量',
        unit: '單位',
        unitCost: '單價',
        lineTotal: '行小計',
        addLine: '新增行',
        removeLine: '刪除',
      },
      totals: {
        invoiceTotal: '發票總金額',
      },
      actions: {
        save: '儲存',
        cancel: '取消',
        close: '關閉',
        refreshFromJobNumbers: '依工單編號重新整理',
        confirmRefresh: '重新產生發票項目',
        newClient: '新增客戶',
      },
      validation: {
        clientRequired: '請選擇客戶。',
        invoiceDateRequired: '請填寫發票日期。',
        dueDateRequired: '請填寫到期日期。',
        atLeastOneLine: '至少需要一個行項目。',
        qtyPositive: '數量必須為 0 或以上。',
        unitCostNonNegative: '單價不能為負數。',
        jobNumberFormat: '不支援的工單編號格式。',
      },
      messages: {
        saveFailed: '儲存發票失敗。',
        loadFailed: '載入發票詳情失敗。',
        refreshFailed: '依工單編號重新整理發票項目失敗。',
        refreshOverwriteConfirm: '您的發票項目已有手動變更。依工單編號重新產生將覆蓋目前資料。要繼續嗎？',
        unresolvedJobs: '以下工單編號無法解析：{jobs}',
        manualReviewJobs: '以下工單編號需要手動檢查，因為無法擷取第 1 節內容：{jobs}',
        manualReviewRow: '需要手動檢查：第 1 節描述為空白。',
      },
    },
  },
} as const