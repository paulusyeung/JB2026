export const crmMessages = {
  staffMember: {
    form: {
      newTitle: 'New Staff Member',
      editTitle: 'Staff Member',
      email: 'Email',
      invalidEmail: 'Invalid email format',
      emailInUse: 'This email is already in use by another staff member',
    },
    actions: {
      new: 'New Staff Member',
    },
  },
  companies: {
    lookup: 'Search companies...',
    headers: {
      name: 'Company Name',
      synced: 'Synced',
      accountOwner: 'Account Owner',
      domainName: 'Domain Name',
      address: 'Address',
      people: 'People',
      opportunities: 'Opportunities',
      createdOn: 'Created on',
      createdBy: 'Created by',
      updatedOn: 'Updated on',
      updatedBy: 'Updated by',
    },
    form: {
      editTitle: 'Edit Company',
      name: 'Company Name',
      domainName: 'Domain Name',
      address: 'Address',
      addressStreet1: 'Address Line 1',
      addressStreet2: 'Address Line 2',
      city: 'City',
      state: 'State',
      postcode: 'Postal Code',
      country: 'Country',
      save: 'Save',
      saveClose: 'Save & Close',
      cancel: 'Cancel',
      delete: 'Delete',
      requiredName: 'Company name is required',
      addPerson: 'Add person',
      addOpportunity: 'Add opportunity',
      migrateCustomer: 'Migrate Customer',
      migrateCustomerSelect: 'Select JB2026 Customer',
    },
    actions: {
      columns: 'Columns',
      sorting: 'Sorting',
      sortBy: 'Sort by',
      asc: 'A-Z',
      desc: 'Z-A',
      checkbox: 'Checkbox',
      views: 'Views',
      newCompany: 'New Company',
      detailView: 'Detail View',
      cardView: 'Card View',
      selected: '{count} selected',
    },
    messages: {
      loadFailed: 'Failed to load companies',
      loadRecordFailed: 'Failed to load company details',
      saveSuccess: 'Company saved successfully',
      saveFailed: 'Failed to save company',
      syncedTooltip: 'Synced to Twenty CRM',
    },
  },
  people: {
    lookup: 'Search people...',
    headers: {
      synced: 'Synced',
      name: 'Name',
      email: 'Email',
      phone: 'Phone',
      jobTitle: 'Job Title',
      company: 'Company',
      createdOn: 'Created on',
      createdBy: 'Created by',
      updatedOn: 'Updated on',
      updatedBy: 'Updated by',
    },
    actions: {
      columns: 'Columns',
      sorting: 'Sorting',
      sortBy: 'Sort by',
      asc: 'A-Z',
      desc: 'Z-A',
      checkbox: 'Checkbox',
      views: 'Views',
      newPerson: 'New People',
      detailView: 'Detail View',
      cardView: 'Card View',
      selected: '{count} selected',
    },
    messages: {
      loadFailed: 'Failed to load people',
      loadRecordFailed: 'Failed to load person details',
      saveSuccess: 'Person saved successfully',
      saveFailed: 'Failed to save person',
      syncedTooltip: 'Synced to Twenty CRM',
    },
    form: {
      editTitle: 'Edit People',
      name: 'Name',
      jobTitle: 'Job Title',
      email: 'Email',
      phone: 'Phone',
      company: 'Company',
      addEmail: 'Add email',
      addPhone: 'Phone number',
      phoneHint: 'Include the country code, e.g. +14155552671',
      invalidPhone: 'Enter a valid phone number for {country}',
      save: 'Save',
      saveClose: 'Save & Close',
      cancel: 'Cancel',
      requiredName: 'Name is required',
    },
  },
  tasks: {
    lookup: 'Search tasks...',
    headers: {
      title: 'Title',
      status: 'Status',
      body: 'Body',
      dueDate: 'Due Date',
      assignee: 'Assignee',
      relations: 'Relations',
      createdOn: 'Created on',
      createdBy: 'Created by',
      updatedOn: 'Updated on',
      updatedBy: 'Updated by',
    },
    actions: {
      columns: 'Columns',
      sorting: 'Sorting',
      sortBy: 'Sort by',
      asc: 'A-Z',
      desc: 'Z-A',
      checkbox: 'Checkbox',
      views: 'Views',
      detailView: 'Detail View',
      cardView: 'Card View',
      selected: '{count} selected',
      newTask: 'New Task',
    },
    messages: {
      loadFailed: 'Failed to load tasks',
      loadRecordFailed: 'Failed to load task details',
      saveSuccess: 'Task saved successfully',
      saveFailed: 'Failed to save task',
    },
    form: {
      newTitle: 'New Task',
      editTitle: 'Edit Task',
      title: 'Title',
      body: 'Body',
      status: 'Status',
      dueDate: 'Due Date',
      assignee: 'Assignee',
      save: 'Save',
      saveClose: 'Save & Close',
      cancel: 'Cancel',
      requiredTitle: 'Title is required',
    },
    status: {
      notStarted: 'Not Started',
      inProgress: 'In Progress',
      completed: 'Completed',
      deferred: 'Deferred',
    },
  },
  opportunities: {
    lookup: 'Search opportunities...',
    headers: {
      name: 'Name',
      stage: 'Stage',
      closeDate: 'Closed On',
      amount: 'Amount',
      company: 'Company',
      pointOfContact: 'Point of Contact',
      owner: 'Owner',
      createdOn: 'Created on',
      createdBy: 'Created by',
      updatedOn: 'Updated on',
      updatedBy: 'Updated by',
    },
    actions: {
      columns: 'Columns',
      sorting: 'Sorting',
      sortBy: 'Sort by',
      asc: 'A-Z',
      desc: 'Z-A',
      checkbox: 'Checkbox',
      views: 'Views',
      detailView: 'Detail View',
      cardView: 'Card View',
      selected: '{count} selected',
      newOpportunity: 'New Opportunity',
    },
    messages: {
      loadFailed: 'Failed to load opportunities',
      loadRecordFailed: 'Failed to load opportunity details',
      saveSuccess: 'Opportunity saved successfully',
      saveFailed: 'Failed to save opportunity',
    },
    form: {
      newTitle: 'New Opportunity',
      editTitle: 'Edit Opportunity',
      name: 'Name',
      stage: 'Stage',
      closeDate: 'Closed On',
      amount: 'Amount',
      save: 'Save',
      saveClose: 'Save & Close',
      cancel: 'Cancel',
      requiredName: 'Name is required',
      invalidCurrency: 'Must be a 3-letter currency code (e.g. USD)',
    },
  },
} as const
