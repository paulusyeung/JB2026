  ...
  <section class="page-section admin-user-page">
    <v-card rounded="xl" elevation="0" class="panel-card admin-user-card">
      ...
      <v-card-text>
        ...
        <div v-else class="admin-user-table-shell">
        <v-data-table
          :headers="headers"
          :items="displayedRows"
          :loading="loading"
        ...
        >
          ...
          <template #[`item.username`]='{ item }'>
            <v-btn variant="text" color="primary" density="compact" class="px-0 text-none text-start" style="min-width: auto" @click.stop="openPopup(item.userId)">
              {{ item.username }}
            </v-btn>
          </template>
