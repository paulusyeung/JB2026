...
- [ ] 1.1 Add `INVOICE_NINJA_API_KEY` and `INVOICE_NINJA_BASE_URL` to backend environment configuration.
...
- [ ] 2.1 Define and implement the customer mapping contract from JB2026 customer data to Invoice Ninja client payloads.
...
- [ ] 2.5 Add "Sync with Billing" and billing status affordances to `AdminCustomerView` and the customer service layer.
...
- [ ] 3.2 Implement the deterministic mapping from `JobOrder` data to Invoice Ninja invoice payloads and line items.
...
- [ ] 5.1 Verify end-to-end flow: Sync Customer $\rightarrow$ Create Job $\rightarrow$ Generate Invoice $\rightarrow$ Observe status in Billing and Job screens.
